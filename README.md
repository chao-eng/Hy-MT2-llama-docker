# Hy-MT2 翻译服务部署方案 (桌面端 & Docker)

> 🐳 小型轻量级腾讯混元 Hy-MT2 翻译模型的开箱即用部署方案。当前支持 **桌面客户端** 与 **Docker 容器化** 两种部署模式，均基于 llama.cpp 推理引擎，提供超低资源占用、高品质的本地流式翻译与 OpenAI 兼容的 API 服务。

## 🖥️ 桌面客户端部署 (Tauri 桌面端)

针对普通用户和本地调试，项目提供了一款基于 **Tauri + HTML/CSS/JS** 开发的轻量级绿色桌面客户端：

- ⚡ **开箱即用**：内置 Windows / macOS 双平台的高性能推理引擎侧边栏 (Sidecar)，无需安装 Docker。
- 📂 **模型自由切换**：支持本地扫描 `.gguf` 模型目录，切换模型后自动热重启翻译引擎。
- ⚙️ **核心参数配置**：支持可视化调整推理线程数、最大上下文长度（附带中文大约字数估算）以及自定义系统提示词（System Prompt）模板。
- 💡 **独立推荐指令**：内置官方推荐的 7 大场景翻译提示词模板（默认翻译、术语控制、风格、结构化数据等），支持一键复制。
- 🎯 **双控快捷开关**：在主翻译界面与设置界面均提供引擎启停的快捷开关，并实现进程与主窗口生命周期绑定（主窗口关闭时自动清理后台引擎，防止端口占用）。

### 运行与打包
桌面客户端源代码位于 [/desktop](file:///e:/UGit/Hy-MT2-llama-docker/desktop) 目录中：
1. **启动开发调试**：`npm run tauri dev`（处于 `desktop` 目录下）
2. **打包生成安装包**：`npm run tauri build`

*同时，本项目配置了 GitHub Actions 自动化工作流，支持在 Actions 页面手动运行工作流打包 Windows `.exe/.msi` 以及 macOS ARM `.dmg/.app` 安装文件。*
