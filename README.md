...
- ⚡ **CPU 指令集加速**，开启 `-O3 -march=native` + OpenMP 多线程并行
...
1. **指令集加速**：`-O3 -march=native` 针对构建主机 CPU 架构开启最高优化
...
- **`-march=native`**：编译产物针对构建主机 CPU 优化。若需跨架构部署，请替换为通用指令集（如 `-march=x86-64-v3`）
